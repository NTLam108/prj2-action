...
# Lọc các trạm ở Hà Nội (có thể điều chỉnh theo tên trạm thực tế)
ha_noi_stations = df[df['Station Name'].str.contains('Hà Nội|Ha Noi|Hanoi', case=False, na=False)]

# Kiểm tra nếu không có dữ liệu
if ha_noi_stations.empty:
    print("Không tìm thấy dữ liệu trạm Hà Nội")
    exit()
...
for i, station in enumerate(ha_noi_stations['Station Name'].unique()):
    station_data = ha_noi_stations[ha_noi_stations['Station Name'] == station]
...
plt.title('DIỄN BIẾN CHỈ SỐ PM2.5 TẠI CÁC TRẠM HÀ NỘI', fontsize=18, pad=20, fontweight='bold')
...
output_file = os.path.join(output_dir, f"Hanoi_PM25_Trend_{timestamp}.png")
